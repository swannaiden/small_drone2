...
//Vector3d reference_eul = {-M_PI/2,0,PI}; // Chimera 1 
//Vector3d reference_eul = {0,-M_PI/2,0}; //Taycan
Vector3d reference_eul = {0,0,M_PI}; //Taycan V2
//Vector3d reference_eul = {M_PI,0,M_PI/2.}; // Chimera 2
//Vector3d reference_eul_no_yaw = {M_PI,0,0}; //Chimera 2
Vector3d reference_eul_no_yaw = {0,0,0}; //Taycan
...
void setup() {
  ...
  // Enable new IMU for Smaller drone only
  if (!bno.begin())
  {
  ...
  Serial.print("Initialization of BN005 Succseful");
...
//  for(int i = 0; i <
  ...
  lpassX = new LowPassFilter(.05, .05, 1/100.);
  lpassY = new LowPassFilter(.05, .05, 1/100.);
  lpassZ = new LowPassFilter(.05, .05, 1/100.);
  lpassYaw = new LowPassFilter(.05, .05, 1/100.);
  des_vel.setZero();
  global_vel.setZero();
  ...
  threads.addThread(readIMUBN005_thread);
...
void readSBUS() {
  if(x8r.read(&channels[0], &failSafe, &lostFrame)){
    ...
    if(!isAuto && channels[6] > 1100 && channels[6] < 2000 && millis() - lastAuto > 500 ) {
      ...
      Serial.println("Do Auto ON");


      // lets try just rotating about x (roll)
      double cur_yaw[1];
      quat2yaw(quats_f,cur_yaw);
      Vector3d des_offset = quat2eulZYX()

      Quaterniond offset_quat;
      Matrix3d offset_rotm;
      eul2quat(reference_eul, offset_quat);


      // set gyro offset to zero ****
      //Vector3d reference_eul = {0,0,M_PI}; //Taycan V2
      //Vector3d reference_eul = {M_PI,0,M_PI/2.}; // Chimera 2
      //Vector3d reference_eul_no_yaw = {M_PI,0,0}; //Chimera 2
      //Vector3d reference_eul_no_yaw = {0,0,0}; //Taycan
      ...
      //current 
      //Quaterniond current_offset;
      //current_offset.w() = quats_f[0]; current_offset.x() = quats_f[1]; current_offset.y() = quats_f[2]; current_offset.z() = quats_f[3];
      //Quaterniond temp; temp.w() = reference_quatd.w(); temp.x() = reference_quatd.x(); temp.y() = reference_quatd.y(); temp.z() = reference_quatd.z();



      // double temp[4];
      // memcpy(temp, reference_quat,4*sizeof(double));
      // double quats_d[4] = {quats_f[0],quats_f[1],quats_f[2],quats_f[3]};

      // quatmultiply(quats_d, temp, reference_quat);
      // reference_quatd.w() = reference_quat[0]; reference_quatd.x() = reference_quat[1]; reference_quatd.y() = reference_quat[2]; reference_quatd.z() = reference_quat[3];
      // quat2eulZYX(reference_quatd, reference_eul);
      // reference_eul_no_yaw[0] = reference_eul[0]; reference_eul_no_yaw[1] = reference_eul[1];
      // // reference_R;
...
void doAuto() {
  ...
  // for taycan drone only, change channel mapping
  ...
  //** for chimera**
  //** for Taycan**
  des_roll_rate = (channels[0]-172)/(1811.-172)*2-1;
  des_pitch_rate = (channels[1]-172)/(1811.-172)*2-1;
...
//  delay(10);
  ...
  // for taycan******
  float des_throttle = (channels[2] - 172.)/((1811.-172.));
  float des_roll = fmin(fmax(cmd[0],-1),1);
  float des_pitch = fmin(fmax(cmd[1],-1),1);
  float des_yaw = fmin(fmax(-cmd[2],-1),1);
  ...
  // for taycan *********
  des_channels[2] = channels[2];//(des_throttle*(1811-172))+172;
  des_channels[0] = ((des_roll+1)*((1811-172)/2))+172;
  des_channels[1] = ((des_pitch+1)*((1811-172)/2))+172;
  des_channels[3] = ((des_yaw+1)*((1811-172)/2))+172;
  ...
  // double x0[13] = {lpassX->data_.y,lpassY->data_.y,lpassZ->data_.y,
  //                 quats_f_opti_yaw[3],quats_f_opti_yaw[0],quats_f_opti_yaw[1],quats_f_opti_yaw[2],
  //                 lpassX->data_.yDot,lpassY->data_.yDot,lpassZ->data_.yDot,
  ...
  if ((millis() - lastTime2) > 100)
  {
    ...
    // Serial.print(") x: (");
    // for (int i = 0; i < 3; i++)
    //   {Serial.print(lla_tmp[i]); Serial.print(",");}
    // Serial.print(") v: (");
    // for (int i = 0; i < 3; i++)
...
void readIMUBN005() {
  ...
  // read data from the new IMU
  quatsBN = bno.getQuat();
  quats_f[0] = quatsBN.w(); quats_f[1] = quatsBN.x(); quats_f[2] = quatsBN.y(); quats_f[3] = quatsBN.z();

  //rotate quats
  // chimera
  // double quats_f_rotated[4] = {quats_f[3],quats_f[0],quats_f[1],quats_f[2]};
  // taycan
  double quats_f_rotated[4] = {quats_f[0],quats_f[1],quats_f[2],quats_f[3]};
  quatmultiply(reference_quat_no_yaw,quats_f_rotated,quat_state);
  double drew[4];
  memcpy(drew,quat_state,4*sizeof(double));
  quatmultiply(drew,reference_quat,quat_state);
  ...
  rates_f[0] = angVelocityData.gyro.x; rates_f[1] = angVelocityData.gyro.y; rates_f[2] = angVelocityData.gyro.z;
...
void readXbee() {
  ...
  if(Serial8.available() > 0) {
    // Serial8.readBytes(message, 2);
    // Serial.println(message);
    xbeeCount++;
    // Serial.print((char) message[0]+"");     Serial.println((char) message[1]+""); 
    ...
    // Serial.print("RAW: ");
    // Serial.print(lpassX->data_.yDotRaw); Serial.print(",");
    // Serial.print(lpassY->data_.yDotRaw); Serial.print(",");
    // Serial.print(lpassZ->data_.yDotRaw); Serial.print(",");
    // Serial.print(lpassYaw->data_.yDotRaw); Serial.println("");
